// ==UserScript==
// @name        Pirate Bay Cleaner 2
// @description	Only runs on the pirate bay website. Based on userscript The Pirate Bay Cleaner by BoKu Auto Sorting, Torrentifying, Theme Change, Search Change, SSL/HTTPS and more...
// @namespace   
// @icon        http://i50.photobucket.com/albums/f303/jonjoe15/large_zpsafdbedad.jpg
// @license 	Pirate Bay Cleaner 2 is licensed under a Creative Commons Attribution-ShareAlike 3.0 Unported License.
// @author      Jg5050
// @contributor stuK & Boku
// @version     2.0
...
var GoogleSiteID = $('meta[name=google-site-verification]').attr("content");
    ...
    https=GM_getValue("https",true);
        if(sitedomain != 'thepiratebay.se'){ https = false; httpsdisabled = '<b>Notice: </b>HTTPS is not enabled on proxy sites'; }
    ...
    function Lightbox(type, url, nobg){
        ...
        switch (type){
            ...
            case 'settings':
                ...
                +'<h1 style="text-align:center;">Pirate Bay Cleaner 2 Settings</h1>'
                        ...
                        $("<label/>", {html: "Client:", style: "margin-left:25px;margin-right:10px;"}).appendTo('#tbpc_formcontents');
                            var tpbc_remotetorrent_client = document.createElement('select');
                                    ...
                                    tpbc_remotetorrent_client.addEventListener('change', function(){
                                        ...
                                        client_4_url.placeholder = "Example: http://192.168.199.103:8080";
    ...
    /* If use DuckDuckGo has been set, then change fields to show it, so the user knows */
    if(duckduckgo == true){
        $('#chb').remove();
        $('#subm > input:nth-child(2)').remove();
        $('#q > label').remove();
        $('#q > select').remove();
        $('.https_form').attr('placeholder', 'DuckDuckGo Search...');
        $('#inp > input').attr('placeholder', 'DuckDuckGo Search...');
        $('.submitbutton').attr('value', 'DuckDuckGo Search...');
        $('#subm > input:first').attr('value', 'DuckDuckGo Search...');
    }
    ...
    if(stretch == true){
        ...
        $('#main-content').css({'margin-left':'0','padding-right':'0','margin':'0','min-width':'100%'});
    }
    ...
    if(remotetorrent==true){
        ...
        case '1':
            ...
            $('#torrent_client').on('click', function(){ 
                if(remotetorrent_client_mode=='window'){GM_openInTab('https://remote.utorrent.com/srp', 'active');}
        ...
        case '2':
            ...
            $('#torrent_client').on('click', function(){ 
                if(remotetorrent_client_mode=='window'){GM_openInTab('http://remote.vuze.com/', 'active');}
                else if(remotetorrent_client_mode=='lightbox'){
                    LightboxBG();
        ...
        case '3':
            ...
            $('#torrent_client').on('click', function(){ 
                if(remotetorrent_client_mode=='window'){GM_openInTab('https://remote.bittorrent.com/srp', 'active');}
                else if(remotetorrent_client_mode=='lightbox'){
                    LightboxBG();
        ...
        case '4':
            ...
            $('#torrent_client').on('click', function(){ 
                if(remotetorrent_client_mode=='window'){GM_openInTab(transmission_url, 'active');}
                else if(remotetorrent_client_mode=='lightbox'){
                    LightboxBG();
    ...
    if(save_faves==true && url.match(/\/user\//gi)){
        ...
        faves_dropdown.addEventListener('change', function(){
            if(this.value != ""){
                location.replace('\/\/' + sitedomain + '\/user\/' + this.value + '\/');
            }
        });
    ...
    $('#searchResult > tbody > tr').each(function(i){
        ...
        if(!torrentHTML.match(/alt="Next"|alt="Previous"/gi)){
                    ...
                    function(){
                        ...
                        $.ajax({
                            ...
                            success: function(data){
                                ...
                                if(imdb_url) GM_openInTab("http://"+imdb_url[0], 'active'); else { alert('No IMDB Link Found'); }
                            },
                            async: false
                        });
                    }, false);
            ...
            if(torrentify==true){
                if(single==false) var torrentURL = $(this).children('td:nth-child(2)').children('div:first').children('a:first').attr('href').replace('/torrent','//torrents.' + sitedomain); 
                else var torrentURL = $(this).children('td:nth-child(2)').children('a').attr('href').replace('/torrent','//torrents.' + sitedomain);
                var tpbc_torrent_icon = document.createElement('a');
                var linkText = document.createElement('img');
                linkText.src = '//' + sitedomain + '/static/img/dl.gif';
                tpbc_torrent_icon.appendChild(linkText);
                tpbc_torrent_icon.title = 'Download this torrent';
                tpbc_torrent_icon.href = torrentURL + ' [Pirate Bay Cleaner 2].torrent';
                if(save_history==true){
    ...
    if($('#searchResult > tbody > tr').length <= 1){
        ...
        if( $('#searchResult > tbody').length > 0 ){
            ...
            if($('#categoriesTable').length == 0 && $('#details').length == 0 && !url.match(/\/tags|\/doodles|\/downloads|\/policy|\/contact|\/blog|\/legal|\/about|\/language|\/register|\/login|\/ajax|\/rss|\/settings/gi)){
    ...
    if(httpsdisabled){
        var https_disabled_text = document.createElement('p');
        https_disabled_text.style.cssText = 'display:block; float:right; margin-right:5px; color:#c0c0c0;';
        https_disabled_text.innerHTML = httpsdisabled;
        $('#tpbc_btn_container').append(https_disabled_text);
    }
    ...
    document.title += " - Empowered by Pirate Bay Cleaner 2";
}
